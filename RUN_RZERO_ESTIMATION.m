...
for sweep_ind=1:num_dx
    ...
    parfor rep=1:nreps
        ...
        %% initialise
        %virus
        mesh_weight = 1 + (pde_dx>1)*(pde_dx^2 - 1); %weight to account for virus having 
        ...
        for inf_cell = 1:num_inf_cells
            ...
            if pde_dx>=1 && save_spatial_info
                coords_of_inf_cell_nodes(inf_cell,:) = [ceil(x_c/pde_dx)*pde_dx-0.5*pde_dx, ...
                                                        ceil(y_c/pde_dx)*pde_dx-0.5*pde_dx];
            end
        end
        
        
        infected_dist = get_production_matrix(2, grid, pde_dx, 0*grid+1);
        ...
        if pde_dx>=1 && save_spatial_info
            ...
            for inf_node = 1:length(unique_inf_nodes) %for each infected node
                ...
                num_same_plate_cells(rep) = num_same_plate_cells(rep) + pde_dx^2 - num_inf_cells_at_node;
            end
        end
        ...
        while t<=final_time
                ...
                for j=1:cells_long
                    ...
                    if grid(i,j)==1
                        ...
                        prob_t_to_i = 1-exp(-total_cells*beta_param*...
                            get_local_density(virus,i,j,pde_dx)*dt);
                        ...
                        if rand<prob_t_to_i  %becomes infected
                            ...
                            if pde_dx>=1 && save_spatial_info
                                ...
                                %infected cell
                                new_inf_node_coords = [ceil(i/pde_dx)*pde_dx-0.5*pde_dx, ceil(j/pde_dx)*pde_dx-0.5*pde_dx];
            ...
            vol_weight = 1 + coarse_mesh * (-1 + pde_dx^2);
            ...
            new_virus(2:mesh_width-1, 2:mesh_length-1) = ...
                new_virus(2:mesh_width-1, 2:mesh_length-1) + ...
                virus_diff*(dt/pde_dx^2) * (virus(1:mesh_width-2, 2:mesh_length-1) +...
